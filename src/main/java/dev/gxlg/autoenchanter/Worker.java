...
public class Worker {
    ...
    private static void beginRecovery(MinecraftClient client, ClientPlayerEntity player) {
        if (lastAnvilPos == null || client.world == null) {
            debug(player, "Anvil screen closed but no anvil position was recorded - nothing to recover (this is normal if you closed it yourself, but if the anvil really broke, the position-capture hook may not be working).");
            return;
        }
        if (!client.world.getBlockState(lastAnvilPos).isAir()) {
            debug(player, "Anvil screen closed but block at " + lastAnvilPos.toShortString() + " is still " + client.world.getBlockState(lastAnvilPos).getBlock() + " (not air) - assuming you closed it yourself, not recovering.");
            return;
        }
        ...
        if (offhand.isEmpty() || !(offhand.getItem() instanceof BlockItem blockItem) || !(blockItem.getBlock() instanceof AnvilBlock)) {
            ...
            debug(player, "Recovery aborted: off-hand does not hold an anvil (has: " + offhand.getItem() + ")");
            cancel();
            return;
        }
        ...
        debug(player, "Recovery started: anvil at " + lastAnvilPos.toShortString() + " is now air, off-hand has " + offhand.getItem());
    }
    ...
    private static void handleRecovery(MinecraftClient client, ClientPlayerInteractionManager manager, ClientPlayerEntity player) {
        ...
        if (state == State.RECOVER_PLACE) {
            ...
            debug(player, "Recovery: tried placing anvil at " + lastAnvilPos.toShortString()
                    + " (clicked top of " + below.toShortString() + "), result=" + result
                    + ", off-hand now=" + player.getOffHandStack().getItem());
        ...
        if (state == State.RECOVER_OPEN) {
            ...
            if (!(client.world.getBlockState(lastAnvilPos).getBlock() instanceof AnvilBlock)) {
                ...
                recoverAttempts++;
                debug(player, "Recovery: block at " + lastAnvilPos.toShortString() + " is still "
                        + client.world.getBlockState(lastAnvilPos).getBlock() + " after placing, attempt " + recoverAttempts);
            ...
            debug(player, "Recovery: tried opening anvil at " + lastAnvilPos.toShortString() + ", result=" + result);
