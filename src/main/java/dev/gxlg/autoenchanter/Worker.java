...
public class Worker {
    ...
    // ==== NEW: master on/off switch, toggled with Ctrl+U ====
    ...
    /** Called from the Ctrl+U keybinding handler in AutoEnchanter. */
    public static void toggleSystem(ClientPlayerEntity player) {
        systemEnabled = !systemEnabled;
        if (player != null) {
            player.sendMessage(Text.literal("[AutoEnchanter] System " + (systemEnabled ? "ENABLED" : "DISABLED"))
                    .formatted(systemEnabled ? Formatting.GREEN : Formatting.RED), false);
        }
    }
    ...
    // NEW: remembers which state we were in when the anvil screen disappeared, so
    // recovery resumes the right thing (continue enchanting vs. go back to waiting
    // for the next batch vs. resume an in-progress calculation).
    private static State recoverResumeState = State.EXEC;

    public static void tick() {
        if (!systemEnabled) return; // NEW: master switch (Ctrl+U) - do nothing at all while off.
        ...
        // NEW: independent safety net, runs every tick regardless of what state
        // we're in - if a diamond pickaxe already carrying all 4 required
        // enchantments (Silk Touch, Mending, Efficiency V, Unbreaking III) is
        // sitting on the hotbar, drop it immediately. This does not depend on the
        // internal step-by-step bookkeeping at all, so it always catches a fully
        // enchanted pickaxe that got stranded (e.g. anvil breaking at the exact
        // moment the last enchant landed) even if some other tracking slipped up.
        checkAndDropFinishedTarget(client, manager, player);

        // NEW: handle the "anvil broke" auto-replace routine
        if (state == State.RECOVER_ANVIL) {
            ...
            // run while we were still in this state and skipped resuming things).
            ...
            if (client.currentScreen instanceof AnvilScreen) {
                debug(player, "Recovery: the anvil screen is open now - resuming " + recoverResumeState + ".");
                state = recoverResumeState;
                if (state == State.EXEC) start();
                // AUTO_SCAN and CALCULATE just pick back up naturally on the next
                // pass through their own branches below - nothing else to do here.
                return;
            }
        ...
        // NEW: universal anvil-break detection. Previously this only applied while
        // actively executing a merge (EXEC), so breaking the anvil while merely
        // waiting between batches (AUTO_SCAN) or mid-calculation (CALCULATE) went
        // completely unnoticed - the mod would just sit there forever afterwards
        // since those states never checked whether the screen had disappeared.
        if ((state == State.AUTO_SCAN || state == State.CALCULATE || state == State.EXEC)
                && !(client.currentScreen instanceof AnvilScreen)) {
            sopid = 0;
            beginRecovery(client, player, state);
            return;
        }
     ...
     *
     * NOTE: uses a loose "has at least these enchantments" check rather than the
     * strict EnchantedItem.matches() (which also requires the predicted anvil-use
     * repair-cost value to match exactly) - if the anvil broke at the very moment
     * the merge completed, the item is undeniably already fully merged regardless
     * of whether our repair-cost prediction lines up bit-for-bit with what the
     * server actually assigned, and requiring exact equality here was causing the
     * finished item to go completely unrecognized (and never get thrown out).
     */
    private static void resolveCompletedOperations(ScreenHandler handler, ClientPlayerInteractionManager manager, ClientPlayerEntity player) {
        while (opid < operations.size()) {
            ...
            int slot = IntStream.range(3, handler.getStacks().size()).boxed()
                    .filter(i -> hasAtLeastEnchants(result, handler.getSlot(i).getStack()))
                    .findFirst().orElse(-1);
    ...
    /** Loose check: same item type and carries at least all of the expected enchantments at the expected level (or higher) - ignores the exact repair-cost/anvil-use bookkeeping that EnchantedItem.matches() otherwise requires bit-for-bit. */
    private static boolean hasAtLeastEnchants(EnchantedItem expected, ItemStack stack) {
        if (stack.isEmpty() || expected.item() != stack.getItem()) return false;
        Enchant actual = Enchant.from(stack);
        for (Map.Entry<RegistryEntry<Enchantment>, EMap> e : expected.enchantments().entrySet()) {
            EMap actualEnc = actual.enchantments().get(e.getKey());
            if (actualEnc == null || actualEnc.lvl() < e.getValue().lvl()) return false;
        }
        return true;
    }
    ...
    /**
     * Independent safety net (see call site in tick()): scans the hotbar for a
     * diamond pickaxe that already carries all 4 required enchantments and drops
     * it straight into the world, regardless of what the internal state machine
     * currently thinks is going on. If we were actively mid-plan for that exact
     * slot (CALCULATE/EXEC), that plan is now stale since its target just
     * disappeared - go back to a fresh scan instead of getting stuck.
     */
    private static void checkAndDropFinishedTarget(MinecraftClient client, ClientPlayerInteractionManager manager, ClientPlayerEntity player) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            ItemStack stack = handler.getSlot(slot).getStack();
            if (stack.isEmpty() || stack.getItem() != TARGET_ITEM) continue;
            if (!hasAllRequiredEnchants(stack)) continue;

            manager.clickSlot(handler.syncId, slot, 0, SlotActionType.PICKUP, player);
            if (!handler.getCursorStack().isEmpty()) {
                manager.clickSlot(handler.syncId, -999, 0, SlotActionType.PICKUP, player);
            }
            debug(player, "Safety check: found a fully-enchanted pickaxe on the hotbar (slot " + i + "), dropped it.");

            if (state == State.EXEC || state == State.CALCULATE) {
                beginAutoScan("Dropped a fully-enchanted pickaxe, rescanning...");
            }
            return; // one per pass is plenty - a later pass will catch any others
        }
    }

    /** True if the stack carries every enchantment in REQUIRED_ENCHANTS at the required level or higher. */
    private static boolean hasAllRequiredEnchants(ItemStack stack) {
        Enchant actual = Enchant.from(stack);
        for (Map.Entry<RegistryKey<Enchantment>, Integer> req : REQUIRED_ENCHANTS) {
            boolean found = actual.enchantments().entrySet().stream()
                    .anyMatch(e -> e.getKey().matchesKey(req.getKey()) && e.getValue().lvl() >= req.getValue());
     ...
     */
    private static void beginRecovery(MinecraftClient client, ClientPlayerEntity player, State resumeState) {
        recoverResumeState = resumeState;
        state = State.RECOVER_ANVIL;
        recoverTimer = 0;
        recoverAttempts = 0;
        showText("Anvil screen closed, recovering...", Colors.RED);
        debug(player, "Recovery started (was in " + resumeState + ") - will act on whatever block you're currently looking at.");
    }
    ...
    private static void handleRecovery(MinecraftClient client, ClientPlayerInteractionManager manager, ClientPlayerEntity player) {
        ...
        if (offhand.isEmpty() || !(offhand.getItem() instanceof BlockItem blockItem) || !(blockItem.getBlock() instanceof AnvilBlock)) {
            recoverAttempts++;
            // NEW: this needs the player's attention (placing consumes 1 anvil from
            // the off-hand stack each time, so it WILL run out eventually if not
            // restocked) - keep the on-screen text updated every cycle, and repeat
            // the chat warning every ~2s rather than burying it in throttled debug.
            showText("No anvil in your off-hand - waiting for one...", Colors.RED);
            if (recoverAttempts % 4 == 1) {
                player.sendMessage(Text.literal("[AutoEnchanter] No anvil in your off-hand! Put more anvils there - waiting (attempt " + recoverAttempts + ")").formatted(Formatting.RED), false);
            }
