...
public class Worker {
    ...
    public static void start() {
        ...
        for (EnchantedItem inp : inputs) {
            ...
            if (slot == -1) {
                ...
                // NEW: don't get stuck forever waiting on a plan that no longer
                // matches reality (e.g. items shifted while the anvil was being
                // replaced) - fall back to a fresh hotbar scan instead.
                if (loopEnabled) {
                    beginAutoScan("Lost track mid-enchant, rescanning the hotbar...");
                }
                return;
            }
            found.add(slot);
    ...
    private static void handleRecovery(MinecraftClient client, ClientPlayerInteractionManager manager, ClientPlayerEntity player) {
        ...
        if (!(client.crosshairTarget instanceof BlockHitResult hit) || hit.getType() != HitResult.Type.BLOCK) {
            recoverAttempts++;
            debug(player, "Recovery: not looking at a block (attempt " + recoverAttempts + ") - look at the anvil.");
            if (recoverAttempts > 40) {
                showText("Look at the anvil so it can recover", Colors.RED);
                recoverAttempts = 0;
            }
            return;
        }
        ...
        if (client.world.getBlockState(pos).isIn(BlockTags.ANVIL)) {
            ...
            // start() automatically to resume exactly where we left off (and start()
            // itself will fall back to a fresh hotbar scan if the resume doesn't
            // line up with what's actually there anymore).
            state = State.EXEC;
            return;
        }
        ...
        if (offhand.isEmpty() || !(offhand.getItem() instanceof BlockItem blockItem) || !(blockItem.getBlock() instanceof AnvilBlock)) {
            ...
            debug(player, "Recovery: not looking at an anvil and off-hand has no anvil (has " + offhand.getItem() + ")");
            cancel();
            return;
        }
        ...
        debug(player, "Recovery: looked-at block " + client.world.getBlockState(pos).getBlock()
                ...
                + "), result=" + result + " (attempt " + recoverAttempts + ")");
        if (recoverAttempts > 10) {
            showText("Couldn't place a new anvil - make sure you're looking at the right spot", Colors.RED);
            cancel();
        }
    }
